...
int open_write_etasascii_head (START_MESSAGE_DATA hdrec_data,
                               int32_t *vids, char *dec_phys_flags, FILE **pfile)
{
    ...
    /* schreibe Kopf des Stimuli-Files */
    fprintf (*pfile, "ETASAsciiItemFile	record\tCrLf\tTab\n");
    fprintf (*pfile, "sampleCount\tXXXXXXXXXXXXXXXXXXXX\n");   // die X's werden spaeter ueberschrieben
    ...
    for (x = 0; vids[x] > 0; x++) {
        char variname[BBVARI_NAME_SIZE];
        if (x != 0) fprintf (*pfile, "\t");
        GetBlackboardVariableName (vids[x], variname, sizeof(variname));
        fprintf (*pfile, "%s", variname);
        fprintf (*pfile, "\\SC:1");
    }
    fprintf (*pfile, "\n");
    ...
    for (x = 0; vids[x] > 0; x++) {
        if (x != 0) fprintf (*pfile, "\t");
        if ((dec_phys_flags != NULL) && (dec_phys_flags[x] == 1)) {
            fprintf (*pfile, "f8");
        } else {
            switch (get_bbvaritype (vids[x])) {
            case BB_BYTE:
                fprintf (*pfile, "s8");
                break;
            case BB_UBYTE:
...
{
    if (!fseek (fh, 47L, SEEK_SET)) {
        char Help[21];
        int x;
        sprintf (Help, "%u", Samples);   // erste 3 Zeilen sin Kopf
        for (x = (int)strlen (Help); x < 20; x++) {
            Help[x] = ' ';
        }
        Help[20] = 0;
        fputs (Help, fh);
...
int write_ringbuff_mcsrec  (FILE *file, RING_BUFFER_COLOMN *stamp, int rpvari_count)
{
    ...
    status = !fwrite (&filler, sizeof (long), 1, file);

    for (i = 2; ((i < rpvari_count) && !status); i++) {
        switch (stamp->EntryList[i].type) {
        case BB_BYTE:
        ...
        case BB_UBYTE:
            ...
            break;
        case BB_WORD:
            status |= !fwrite (&stamp->EntryList[i].value.w, sizeof (short), 1, file);
            break;
        case BB_UWORD:
            status |= !fwrite (&stamp->EntryList[i].value.uw, sizeof (unsigned short), 1, file);
            break;
        case BB_DWORD:
            status |= !fwrite (&stamp->EntryList[i].value.dw, sizeof (long), 1, file);
            break;
        case BB_UDWORD:
            status |= !fwrite (&stamp->EntryList[i].value.udw, sizeof (unsigned long), 1, file);
            break;
        case BB_QWORD:  // Gredi kann keine 64Bits dann halt nur die unteren 32Bits speichern
            status |= !fwrite (&stamp->EntryList[i].value.dw, sizeof (long), 1, file);
            break;
        case BB_UQWORD:  // Gredi kann keine 64Bits dann halt nur die unteren 32Bits speichern
            status |= !fwrite (&stamp->EntryList[i].value.udw, sizeof (unsigned long), 1, file);
            break;
        case BB_FLOAT:
        ...
        case BB_DOUBLE:
            {
                float tmp = (float)stamp->EntryList[i].value.d;
                status |= !fwrite (&tmp, sizeof (float), 1, file);
            }
            break;
        }
    }
