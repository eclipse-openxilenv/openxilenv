...
#include "StartStopXcpWidget.h"
#include "ui_StartStopXcpWidget.h"
...
int StartStopXCPWidget::Connection = 0;

StartStopXCPWidget::StartStopXCPWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::StartStopXCPWidget),
...
StartStopXCPWidget::~StartStopXCPWidget()
{
    Connection--;
    delete ui;
}

void StartStopXCPWidget::on_pushButtonRight_clicked()
{
...
void StartStopXCPWidget::on_pushButtonLeft_clicked()
{
    QModelIndexList selectedIndexes;
...
void StartStopXCPWidget::on_pushButtonFilter_clicked()
{
    FillListBoxes(QStringToConstChar(ui->lineEditFilter->text()), false);
}

void StartStopXCPWidget::on_pushButtonStart_clicked()
{
    int i;
    ...
    if (!ui->listWidgetOnVar->count()) {
        ...
        Start_XCP (curConnection, START_MEASSUREMENT, i, Pointers);
        my_free(Buffer);
        my_free(Pointers);
...
void StartStopXCPWidget::on_pushButtonStop_clicked()
{
    Stop_XCP (curConnection, STOP_MEASSUREMENT);
    CloseDialog();
}

bool StartStopXCPWidget::InOnList(QString &Name)
{
...
void StartStopXCPWidget::CloseDialog()
{
...
void StartStopXCPWidget::FillListBoxes(const char* Filter, bool UpdateSelected)
{
    int vid;
    int Fd = ScQt_GetMainFileDescriptor();
