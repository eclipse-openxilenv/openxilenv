...
#include "StartStopXcpCalWidget.h"
#include "ui_StartStopXcpCalWidget.h"
...
int StartStopXCPCalWidget::Connection = 0;

StartStopXCPCalWidget::StartStopXCPCalWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::StartStopXCPCalWidget),
...
StartStopXCPCalWidget::~StartStopXCPCalWidget()
{
    Connection--;
    delete ui;
}
...
void StartStopXCPCalWidget::on_pushButtonLeft_clicked()
{
    QModelIndexList selectedIndexes;
...
void StartStopXCPCalWidget::on_pushButtonStart_clicked()
{
    int i;
    ...
    if (!ui->listWidgetOnVar->count()) {
        ...
        return;
    } else if (ui->listWidgetOnVar->count() > 100) {
        ThrowError (1, "max. 100 parameter can be selected");
        return;
    } else {
        ...
        Start_XCP (curConnection, START_CALIBRATION, i, Pointers);
        my_free(Buffer);
        my_free(Pointers);
...
void StartStopXCPCalWidget::on_pushButtonStop_clicked()
{
    Stop_XCP (curConnection, STOP_CALIBRATION);
    CloseDialog();
}

bool StartStopXCPCalWidget::InOnList(QString &Name)
{
...
void StartStopXCPCalWidget::CloseDialog()
{
