...
DEFINE_CMD_CLASS(cGosubCmd)


int cGosubCmd::SyntaxCheck (cParser *par_Parser)
{
    int Idx;
    int FileOffset;
    int LineNr;
    ...
    // If there a one ore more not solved environment variable(s) try to solve them
    if (par_Parser->GetNumNoneSolvedEnvVars (0) || par_Parser->GetNumSolvedEnvVars (0)) {
        ...
        if (par_Parser->GetNumNoneSolvedEnvVars (0)) {
            par_Parser->Error (SCRIPT_PARSER_WARNING, "GOSUB \"%s\" which includes an unknown environment variable at compile time, try to resolve it at run time", par_Parser->GetParameter (0));
        }
...
int cGosubCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    int FromPos;
    int ToPos;
    int GotoIp;
...
int cGosubCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(par_Parser);
    UNUSED(par_Executor);
    UNUSED(Cycles);
...
static cGosubCmd GosubCmd ("GOSUB",                        
                          1, 
                          1,  
                          nullptr,
                          FALSE, 
