...
DEFINE_CMD_CLASS(cSaveSatvlCmd)


int cSaveSatvlCmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}

int cSaveSatvlCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    UNUSED(par_Executor);
    ...
    if (!ScriptWriteProcessToSATVL (par_Parser->GetParameter (0),
                                    par_Parser->GetParameter (1),
...
int cSaveSatvlCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(par_Parser);
    UNUSED(par_Executor);
    UNUSED(Cycles);
...
static cSaveSatvlCmd SaveSatvlCmd ("SAVE_SATVL",
                               2, 
                               3,  
                               nullptr,
                               FALSE, 
