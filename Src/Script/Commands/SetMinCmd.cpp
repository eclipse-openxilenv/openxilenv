...
DEFINE_CMD_CLASS(cSetMinCmd)


int cSetMinCmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}

int cSetMinCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    UNUSED(par_Executor);
    int Vid;
    double Value;
    ...
    set_bbvari_min (Vid, Value);
    return 0;
}

int cSetMinCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(par_Parser);
    UNUSED(par_Executor);
    UNUSED(Cycles);
...
static cSetMinCmd SetMinCmd ("SET_MIN",                        
                             2, 
                             2,  
                             nullptr,
                             FALSE, 
