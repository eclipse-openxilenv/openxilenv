...
DEFINE_CMD_CLASS (cStartCcpCal2Cmd)

int cStartCcpCal2Cmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}

int cStartCcpCal2Cmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    int Connection;
    ...
    if ((Connection < 0) || (Connection >= 4)) {
        par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "START_CCP_CAL2 channel number (%i) out of range (0...3)", Connection);
        return -1;
    }
    if(Start_CCP (Connection, START_CALIBRATION, ParamCount-1, ParamPtrs) != FALSE) {
        par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "START_CCP_CAL2 not possible ! What about CCP-process ?");
        return -1;
    }
...
int cStartCcpCal2Cmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(Cycles);
    int ccp_error;
    char *Help;
    if (Is_CCP_CommandDone (par_Executor->GetData (), &ccp_error, &Help) != FALSE) {
        if (par_Parser->GenDebugFile ()) par_Parser->PrintDebugFile ("\t\t/* START_CCP_CAL2() needed %d cycles */\n", par_Executor->GetWaitCyclesNeeded ());
        if (ccp_error) {
            par_Parser->Error (s_main_ini_val.ScriptStopIfCcpError ? SCRIPT_PARSER_FATAL_ERROR : SCRIPT_PARSER_ERROR_CONTINUE, 
                               "START_CCP_CAL2 command got a protocoll error (%i) = \"%s\"", ccp_error, Help);
        } 
        return 0;
    }
    return 1;
...
static cStartCcpCal2Cmd StartCcpCalCmd ("START_CCP_CAL2",                        
                                        2, 
                                        MAX_PARAMETERS,
                                        nullptr,
                                        FALSE, 
