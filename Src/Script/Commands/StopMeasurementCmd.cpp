...
DEFINE_CMD_CLASS(cStopMeasurementCmd)


int cStopMeasurementCmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}
...
int cStopMeasurementCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    int Connection;
    ...
    if ((Connection < 0) || (Connection >= 4)) {
        par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "STOP_MEASUREMENT channel number (%i) out of range (0...3)", Connection);
        return -1;
    }
    ...
    if (!stricmp ("XCP", par_Parser->GetParameter (0))) {
        ...
        if (Stop_XCP (Connection, STOP_MEASSUREMENT) != FALSE) {
            par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "STOP_MEASUREMENT (XCP) not possible ! What about XCP-process ?");
            return -1;
        }
    } else if (!stricmp ("CCP", par_Parser->GetParameter (0))) {
        par_Executor->SetData (0, STOP_CCP_CMD);
        if (Stop_CCP (Connection, STOP_MEASSUREMENT) != FALSE) {
            par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "STOP_MEASUREMENT (CCP) not possible ! What about CCP-process ?");
            return -1;
        }
   } else {
        int Pid;
        par_Executor->SetData (0, STOP_REF_CMD);
        if ((Pid = get_pid_by_name (par_Parser->GetParameter (0))) >= 0) {  // is it a prozess?
            ScriptDelReferenceVariList (Pid, Connection);
        } else {
            par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "STOP_MEASUREMENT \"%s\" not possible! Allowed are XCP, CCP, or a running process",
...
int cStopMeasurementCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(Cycles);
    int ccp_error;
    char *Help;
    ...
    case STOP_CCP_CMD:
        if (Is_CCP_CommandDone (par_Executor->GetData (), &ccp_error, &Help) != FALSE) {
            if (par_Parser->GenDebugFile ()) par_Parser->PrintDebugFile ("\t\t/* STOP_MEASUREMENT() needed %d cycles */\n", par_Executor->GetWaitCyclesNeeded ());
            if (ccp_error) {
                par_Parser->Error (s_main_ini_val.ScriptStopIfCcpError ? SCRIPT_PARSER_FATAL_ERROR : SCRIPT_PARSER_ERROR_CONTINUE, 
                                   "STOP_MEASUREMENT command got a protocoll error (%i) = \"%s\"", ccp_error, Help);
            } 
            return 0;
        }
        break;
...
static cStopMeasurementCmd StopMeasurementCmd ("STOP_MEASUREMENT",                        
                                               2, 
                                               2,  
                                               nullptr,
                                               FALSE, 
