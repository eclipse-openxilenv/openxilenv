...
DEFINE_CMD_CLASS(cLoadSelCanNodeCmd)


int cLoadSelCanNodeCmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}

int cLoadSelCanNodeCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    UNUSED(par_Executor);
    double erg;
    int Channel;
    ...
    if (par_Parser->SolveEquationForParameter (1, &erg, -1)) {
        ...
        LoadAndSelectCANNodeScriptCommand (Path, Channel);
    }
    return 0;
}

int cLoadSelCanNodeCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(par_Parser);
    UNUSED(par_Executor);
    UNUSED(Cycles);
...
static cLoadSelCanNodeCmd LoadSelCanNodeCmd ("LOAD_SEL_CAN_NODE",                        
                                             2, 
                                             2,  
                                             nullptr,
                                             FALSE, 
