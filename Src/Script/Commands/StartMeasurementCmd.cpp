...
DEFINE_CMD_CLASS(cStartMeasurementCmd)


int cStartMeasurementCmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}
...
int cStartMeasurementCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    int Connection;
    ...
    if ((Connection < 0) || (Connection >= 4)) {
        par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "START_MEASSUREMENT channel number (%i) out of range (0...3)", Connection);
        return -1;
    }
    ...
    if (!stricmp ("XCP", par_Parser->GetParameter (0))) {
        ...
        if (Start_XCP (Connection, START_MEASSUREMENT, ParamCount - 2, ParamPtrs + 2) != FALSE) {
        ...
        if (Start_CCP (Connection, START_MEASSUREMENT, ParamCount - 2, ParamPtrs + 2) != FALSE) {
        ...
        if ((Pid = get_pid_by_name (par_Parser->GetParameter (0))) >= 0) {  // is this a process?
            ScriptAddReferenceVariList (par_Parser->GetParameter (0), Pid, Connection, ParamCount - 2, ParamPtrs + 2);
        } else {
            par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "START_MEASUREMENT (%s) not possible! Allowed are XCP, CCP, AZG, EDIC, or a running process",
...
int cStartMeasurementCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(Cycles);
    int ccp_error;
    char *Help;
    ...
    case START_CCP_CMD:
        if (Is_CCP_CommandDone (par_Executor->GetData (), &ccp_error, &Help) != FALSE) {
            if (par_Parser->GenDebugFile ()) par_Parser->PrintDebugFile ("\t\t/* START_MEASUREMENT() needed %d cycles */\n", par_Executor->GetWaitCyclesNeeded ());
            if (ccp_error) {
...
static cStartMeasurementCmd StartMeasurementCmd ("START_MEASUREMENT",                        
                                                 3, 
                                                 MAX_PARAMETERS,
                                                 nullptr,
                                                 FALSE, 
