...
DEFINE_CMD_CLASS(cSaveToBinCmd)


int cSaveToBinCmd::SyntaxCheck (cParser *par_Parser)
{
    UNUSED(par_Parser);
    return 0;
}

int cSaveToBinCmd::Execute (cParser *par_Parser, cExecutor *par_Executor)
{
    UNUSED(par_Executor);
    ...
    int returnwert = ScriptWriteProcessToBinFile (par_Parser->GetParameter (0),  /* file name of the symbol files  */
                                                  par_Parser->GetParameter (1),  /* file name of the binary files  */
                                                  par_Parser->GetParameter (2),  /* Process name                 */
                                                  ByteOrder,                /* memory type                 */
                                                  (par_Parser->GetParameterCounter () > 4) ? par_Parser->GetParameter (4) : "*"); /* Label filter   */
    if (!returnwert) {
        par_Parser->Error (SCRIPT_PARSER_FATAL_ERROR, "SAVE_TO_BIN not possible !");
        return -1;
    }
    return 0;
}

int cSaveToBinCmd::Wait (cParser *par_Parser, cExecutor *par_Executor, int Cycles)
{
    UNUSED(par_Parser);
    UNUSED(par_Executor);
    UNUSED(Cycles);
...
static cSaveToBinCmd SaveToBinCmd ("SAVE_TO_BIN",                        
                                   2, 
                                   4,  
                                   nullptr,
                                   FALSE, 
