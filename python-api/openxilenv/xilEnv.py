...
from openxilenv.xilEnv_errors import *
...
class XilEnv:
	...
	def __init__(self, openXilEnv_path: Path) -> None:
		...
		if self.__system == "Windows":
			...
			raise XilEnvPlatformError(f"Unsupported OS: {self.__system}")
		
		lib_path = openXilEnv_path.joinpath(lib_name)

		# validate lib path
		if not lib_path.is_file():
			raise XilEnvInstallationError(f"{lib_name} not found at path: {lib_path}")

		# load lib
		try:
			if self.__system == "Windows":
				self.__lib = ct.windll.LoadLibrary(str(lib_path))
			elif self.__system == "Linux":
				self.__lib = ct.cdll.LoadLibrary(str(lib_path))
		except OSError as e:
			if "undefined symbol" in str(e):
				raise XilEnvPlatformError(
					f"Failed to load {lib_path}: missing symbol. "
					f"This usually indicates a platform compatibility issue in the native library.\n"
					f"Original error: {e}"
				) from e
			else:
				raise XilEnvInstallationError(f"Failed to load library {lib_path}: {e}") from e
		...
		if not self.__xilEnvExePath.is_file():
			raise XilEnvInstallationError(f"XilEnv application not found at path: {self.__xilEnvExePath}")
		if not self.__xilEnvGuiExePath.is_file():
			raise XilEnvInstallationError(f"XilEnvGui application not found at path: {self.__xilEnvGuiExePath}")
